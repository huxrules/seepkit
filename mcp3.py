...
def MCP4_wcd_reader(procqueue,wcdFileString, ProjectUUID, linestatus,dataAvailable, procnum, errorQ ): #wcdFileString, cur ,conn login info needs all the stuff for SDS.
	...
	try:
		...
		#vessel_ID = 1 # need to make the junction table for vessels 

		cur,conn = SDS.LoginData()
		conn.set_session(autocommit=True)
		cur, conn, vessel_ID = SDS.ReadVessel(cur, conn, ProjectUUID)
		
		cur, conn, lineUUID = SDS.WriteLineName(cur, conn, line_name, file_name, vessel_ID, linestatus, dataAvailable, ProjectUUID)
